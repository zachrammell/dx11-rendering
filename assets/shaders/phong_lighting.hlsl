...
vs_out vs_main(vs_in input)
{
  ...
  const float3 view = normalize(CameraPosition.xyz - position_world.xyz);
  ...
  case TP_CYLINDRICAL:
  {
    ...
    const float3 pos = position_world.xyz - model_centroid;
  ...
  for (int i = 0; i < LightCount; ++i)
  {
    ...
    case LT_POINT:
    {
      light_contribution point_light = PointLight(
        ...
        position_world.xyz, view, normal
      );
  ...
  output.diffuse_color = all_of_the_lights.diffuse;
  output.specular_color = all_of_the_lights.specular;

  output.diffuse_color = Fog(output.diffuse_color, float3(1, 1, 1), length(CameraPosition.xyz - position_world.xyz), 0.001f, 10.0f);
  output.specular_color = Fog(output.specular_color, float3(1, 1, 1), length(CameraPosition.xyz - position_world.xyz), 0.001f, 10.0f);
  output.uv = uv;
...
{
  float3 color = TextureDiffuse.Sample(Sampler, input.uv) * input.diffuse_color + TextureSpecular.Sample(Sampler, input.uv).r * input.specular_color;
