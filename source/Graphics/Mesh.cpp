...
Mesh::Mesh(GfxDevice_DX11& render, Model const& m)
...
Mesh::Mesh(GfxDevice_DX11& render, std::vector<Model::Vertex> const& vertex_buffer)
...
void Mesh::Reload(Model const& m, GfxDevice_DX11& render)
{
...
Model Model::GenerateUnitQuad()
{
  Model quad_mesh;

  float3 n { 0, 0, 1 };

  quad_mesh.vertex_buffer.reserve(4);
  // 0 : bottom left
  quad_mesh.vertex_buffer.push_back({ {-1, -1, 0}, n, {0, 1} });
  // 1 : bottom right
  quad_mesh.vertex_buffer.push_back({ { 1, -1, 0}, n, {1, 1} });
  // 2 : top left
  quad_mesh.vertex_buffer.push_back({ {-1,  1, 0}, n, {0, 0} });
  // 3 : top right
  quad_mesh.vertex_buffer.push_back({ { 1,  1, 0}, n, {1, 0} });
  ...
  quad_mesh.index_buffer.reserve(ARRAYSIZE(sequence));
  for (int i = 0; i < ARRAYSIZE(sequence); ++i)
  {
    quad_mesh.index_buffer.push_back(sequence[i]);
  }
