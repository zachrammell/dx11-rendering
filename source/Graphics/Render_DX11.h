...
public:
  GfxDevice_DX11(Platform::OS_Win32& os);
  ~GfxDevice_DX11() = default;

  void SetClearColor(float3 c);
  void SetClearColor(float4 c);

  void Present();
  void ResizeFramebuffer(Platform::OS_Win32 const& os);
...
private: /* ==== Encapsulated DirectX resources ==== */
  ...
  void InitSampler();
  void InitRasterizer();
  void InitViewport(Platform::OS_Win32 const& os);

  void SetupViewport(Platform::OS_Win32 const& os);
  void CleanupRenderTarget();
  void DeleteDepthBuffer();
  void CreateDepthBuffer(Platform::OS_Win32 const& os);
  void CreateRenderTarget(Platform::OS_Win32 const& os);
